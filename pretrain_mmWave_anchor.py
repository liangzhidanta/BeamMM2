...
def mmwave_gps_joint_pretrain(checkpoint_path=None, optimizer_path=None, start_epoch=0, best_loss=None, anchor_modality='vision'):
    ...
    if checkpoint_path is not None and os.path.exists(checkpoint_path):
        model = ImageBindModel(video_frames=8).to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            with amp.autocast(enabled=use_amp):
                ...
                elif anchor_modality == 'gps':
                    loss_gps_vision = info_nce_loss(gps_features, vision_features)
                    loss_gps_mmwave = info_nce_loss(gps_features, mmwave_features)
                    loss = loss_gps_vision + loss_gps_mmwave

            epoch_loss.append([loss_vision_mmwave.item() if anchor_modality == 'vision' else loss_mmwave_vision.item() if anchor_modality == 'mmwave' else loss_gps_vision.item(), 
                               loss_vision_gps.item() if anchor_modality == 'vision' else loss_mmwave_gps.item() if anchor_modality == 'mmwave' else loss_gps_mmwave.item(), 
                               loss.item()])
        ...
        print(f'\tTraining Loss Vision-GPS: {np.mean([x[1] for x in epoch_loss])}')
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                val_loss.append([loss_vision_mmwave.item() if anchor_modality == 'vision' else loss_mmwave_vision.item() if anchor_modality == 'mmwave' else loss_gps_vision.item(), 
                                 loss_vision_gps.item() if anchor_modality == 'vision' else loss_mmwave_gps.item() if anchor_modality == 'mmwave' else loss_gps_mmwave.item(), 
                                 loss.item()])
...
if __name__ == '__main__':
    ...
    anchor_modality='mmwave',
    checkpoint_path='path_to_checkpoint.pth',
    optimizer_path='path_to_optimizer.pth'
)