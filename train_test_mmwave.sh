...
# 模型1相关参数设置（加载预训练编码器1）
###############################
CHECKPOINT_DIR1="./fig1/imagebind_s8_b"
PRETRAINED_ENCODER_PATH1="/home/wzj/BeamMM/checkpoints/vision_s8.pth"  # MMWAVE ANCHOR
...
python -u train_MMWaveEncoderDecoderModel.py \
    --loss "$LOSS" \
    --epochs "$EPOCHS" \
...
# 模型1训练结束后，评估最佳模型（脚本中默认保存最佳模型为 mmwave_encoder_decoder_best.pth）
BEST_MODEL_PATH1="${CHECKPOINT_DIR1}/mmwave_encoder_decoder_best.pth"
if [ -f "$BEST_MODEL_PATH1" ]; then
    echo "开始评估模型1：$BEST_MODEL_PATH1"
    python -u evaluate_MMWaveEncoderDecoderModel.py --model_path "$BEST_MODEL_PATH1"  --dataset_start_idx 1 --dataset_end_idx 9
else
