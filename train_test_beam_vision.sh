...
# 模型1相关参数设置（加载预训练编码器1）
###############################
CHECKPOINT_DIR1="./fig1/imagebind_s4_bv"
PRETRAINED_ENCODER_PATH1="/home/wzj/BeamMM/checkpoints/mmwave_gps_joint_s8.pth"  # MMWAVE ANCHOR
...
python -u train_BeamVisionEncoderDecoderModel.py \
    --loss "$LOSS" \
    --epochs "$EPOCHS" \
...
if [ -f "$BEST_MODEL_PATH1" ]; then
    ...
    python -u evaluate_BeamVisionEncoderDecoderModel.py --model_path "$BEST_MODEL_PATH1"  --dataset_start_idx 1 --dataset_end_idx 9
else
