...
def main():
    ...
    # 定义数据集路径
    dataset_path = [f'/new_disk/yy/DeepSensePre/Data_raw/scenario{i}/' for i in range(args.dataset_start_idx, args.dataset_end_idx)]  # scenario1 ~ scenario9
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            best_model_path = os.path.join(args.checkpoint_dir, 'mmwave_encoder_decoder_best.pth')
        ...
        # 每隔若干个 epoch 保存模型
        if (epoch + 1) % 5 == 0:
            checkpoint_path = os.path.join(args.checkpoint_dir, f'mmwave_encoder_decoder_epoch_{epoch+1}.pth')
